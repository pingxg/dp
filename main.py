...
def nav_to_purchase_invoice(driver):
    purchase_invoices = wait_for_element(
        driver, (By.ID, "Menu1-menuItem003"), "navigate to purchase invoices"
    )
...
def filtering_invoice(
    driver, company="Spartao", status="All", supplier=False, invoice_num=False
):
    ...
    with iframe_context(driver, "main_iframe"):
        select_company = Select(
            wait_for_element(
                ...
                "change company selection to all",
                clickable=False,
            )
        ...
        select_status = Select(
            wait_for_element(
                ...
                "change process invoice filter status",
                clickable=False,
            )
        ).select_by_visible_text(status)
        update_btn = wait_for_element(
            driver,
...
def try_click_invoice_button(
    driver, vendor, invoice_num, statuses=["Data Incomplete", "All"]
):
    """
    ...
    for status in statuses:
        try:
            filtering_invoice(
                driver, supplier=vendor, status=status, invoice_num=invoice_num
            )
            with iframe_context(driver, "main_iframe"):
                ...
                xpath = (
                    ...
                    if status == "All"
                    else "/html/body/div/form/div[4]/table/tbody/tr[2]/td[12]/a"
                )
                invoice_button = wait_for_element(
                    driver,
                    (By.XPATH, xpath),
                    f"attempting find invoice button with status {status}",
                )
    ...
    logging.error(
        f"Failed to find the invoice button for invoice number: {invoice_num}"
    )
    return False
...
def get_invoice_text(driver, vendor, invoice_num):
    ...
    try:
        with iframe_context(driver, "error_iframe"):
            override_lock_status_button = wait_for_element(
                driver,
                (By.XPATH, "/html/body/div/div[4]/input[1]"),
                "try to override the lock frame and click",
                silent=True,
            ).click()
    except:
        pass
    ...
    with iframe_context(driver, "viewer_iframe"):
        try:
            save_pdf_btn = wait_for_element(
                driver,
                (By.ID, "open-button"),
                "find the pdf attachement save btn",
                clickable=False,
                silent=True,
            ).click()
        except (NoSuchElementException, TimeoutException):
            logging.debug(
                "Save PDF button not found or not clickable, but continuing since the file downloads successfully."
            )
        time.sleep(2)
    ...
    with iframe_context(driver, "info_iframe"):
        organizational_unit_btn = wait_for_element(
            driver,
            (
                By.XPATH,
                "/html/body/form/div[3]/div[1]/div[2]/table/tbody/tr[2]/td[2]/div/div/button",
            ),
            "find the organizational unit and click",
        ).click()

    with iframe_context(driver, "dialog_iframe"):
        select_org_unit_btn = wait_for_element(
            driver,
            (By.XPATH, "/html/body/div/div[2]/form/div[4]/input[1]"),
            "select the default org unit",
        ).click()

    with iframe_context(driver, "info_iframe"):
        if vendor == "1381774" or vendor == "1433275":
            due_date = wait_for_element(
                driver,
                (
                    By.XPATH,
                    "/html/body/form/div[3]/div[1]/div[2]/table/tbody/tr[14]/td[2]/div[1]/div/div/input",
                ),
                "modifing the S-Card/Kesko invoices due date to current date",
            ).send_keys(dt.datetime.today().strftime("%d.%m.%Y"))
        if vendor == "1301716":
            ref_field = wait_for_element(
                driver,
                (By.ID, "ReferenceCtrl"),
                "find the ref number and clear the field",
            ).clear()
            msg_field = wait_for_element(
                driver,
                (By.ID, "MessageCtrl"),
                "find the ref number and clear the field",
            ).send_keys(invoice_num)
    ...
    try:
        with iframe_context(driver, "error_iframe"):
            next_time_check = wait_for_element(
                driver,
        ...
        with iframe_context(driver, "info_iframe"):
            if "approver" in posting_info:
                ...
                processor_btn = wait_for_element(
                    ...
                    "find the processor button and click",
                ).click()
        ...
        with iframe_context(driver, "dialog_iframe"):
            current_processor_select = Select(
                wait_for_element(
                    ...
                    "find the current processor select",
                )
            )
            ...
            for option in current_processor_select.options:
                if option.text == f"Approve: {posting_info['approver']}":
                    ...
                    cancel_btn = wait_for_element(
                        driver,
                        (By.XPATH, "/html/body/div/div[2]/form/div[4]/input[2]"),
            ...
            if reset_processor:
                ...
                add_processor_btn = wait_for_element(
                    ...
                    "find the add processor button and click",
                ).click()
                conform_processor = wait_for_element(
                    driver,
                    (By.XPATH, "/html/body/div/div[2]/form/div[4]/input[1]"),
    ...
    with iframe_context(driver, "info_iframe"):
        post_btn = wait_for_element(
            ...
            "find the post button and click",
        ).click()
    ...
    try:
        with iframe_context(driver, "action_iframe"):
            delete_posting = wait_for_element(
                driver, (By.ID, "DeletePostingButton"), "click delete posting value"
            ).click()
    ...
    if vendor != "1301716":
        with iframe_context(driver, "posting_iframe"):
            posting_sum = Select(
                wait_for_element(
                    driver,
                    (By.ID, "PostingControl1_VATHandlingCtrl"),
                    "find the posting sum",
                )
            )
            if "location" in posting_info:
                if posting_info["location"] == "L526":
                ...
                if posting_info["location"] == "L102":
                    ...
                    if "department" in posting_info:
                        department = wait_for_element(
                            ...
                            "update department",
                        ).send_keys(posting_info["department"])

                    if "class_code" in posting_info:
                        class_code = wait_for_element(
                            driver,
                            (
                                By.XPATH,
                            ...
                            "update class",
                        ).send_keys(posting_info["class_code"])
                    location = wait_for_element(
                        ...
                        "update location",
                    ).send_keys(posting_info["location"])
                    ...
                    if "24" in posting_info:
                        ...
                        if posting_info["24_total"] >= 0:
                            debit = wait_for_element(
                                ...
                                "updating 24% debit amount",
                            ).send_keys(str(posting_info["24_total"]).replace(".", ","))
                ...
                if "24" in posting_info and "14" not in posting_info:
                    ...
                    if posting_info["24_total"] >= 0:
                        ...
                        credit = wait_for_element(
                            ...
                            "updating 24% credit amount only",
                        ).send_keys(str(-posting_info["24_total"]).replace(".", ","))
                    tax_code = wait_for_element(
                        driver,
                        (
    ...
    with iframe_context(driver, "info_iframe"):
        save_inv_btn = wait_for_element(
            driver,
            (By.XPATH, "/html/body/form/div[3]/div[2]/input[1]"),
            "find the save invoice button and click",
        ).click()
    return True
