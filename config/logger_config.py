...
def setup_logging():
    ...
    fh = RotatingFileHandler(
        log_path, maxBytes=config["max_log_size"], backupCount=config["backup_count"]
    )
    ...
    ch.setLevel(
        getattr(logging, config["log_notify_level"].upper())
    )  # Example: Only show ERROR and above in console
